...
## [v1.10.6] - 2026-09-25
> 本版改动来自 Task 11.4（Phase 10 交接）。

### 修改
- **REQ-010 规则 · 花费**：压缩失败（SDK 只给 `compact_result: failed`、不给 token 数）也计价：按这一段见过的最大读入上下文记一笔输入。⑤ 成片卡片、模板页头、客户页累计里有没填单价的任务时标「含未知」。
- **REQ-010 规则 · 凭据不外泄**：PowerShell 按通配列环境变量（`gci env:ANTH*`，数组 / 管道 / 括号 / 续行 / `Environment::` 写法与先 `cd env:` 再列同样算，搜索、echo、heredoc 正文里的「env:」字样不算；刻意混淆按 REQ-003 边界不保证）同样拦；`~/.codex` 按家目录写法通配、整目录操作、先进家目录再用相对路径、路径后面直接跟分隔符同样拦（对整个家目录递归搜索按 REQ-003 边界不保证）；写明这些是按命令文本匹配的尽力拦截，同类的其它拼写以后按低优先级补、不阻塞交付，彻底隔离（系统账户或沙箱）放到后续 Phase 评估；Git Bash 的 `/c/Users/...` 写法与 `C:/Users/...` 一样认；Codex 的 `$CODEX_HOME/auth.json` 写进不许碰的凭据文件。
...
### 修改
- **REQ-011 规则 · 失败判定**：Codex 断流重连时发的 `error` 事件（「Reconnecting... n/m」）是进度提示，不判失败（按字面判会把能出的图判死，依据官方 `exec --json` 事件说明）；`turn.failed` 同样判失败。
- **REQ-011 规则 · 不支持**：参考图超过 4 张与透明背景一样以「不支持」失败，在 plan 阶段就拒，不起 Codex。
- **REQ-011 规则 · 额度与原文（11.1 审查后补）**：stderr 含 `usage limit`（订阅额度用完的原话）同样判失败；带回 Build 错误的 JSONL 末段与错误原文逐行截断；超时杀了仍不退出的，宽限期后照样判超时；写明比例与分辨率只是提示词里的要求。

## [v1.10.3] - 2026-09-25
> 本版改动来自 Task 10.5（Phase 10 真机验收）。

### 修改
- **REQ-010 规则 · 花费**：按单价折算把 SDK 压缩上下文那次调用也算上（按压缩时读入的上下文 token 记输入、摘要 token 记输出）。真机用假端点发现：继续一条上下文很长的会话时 SDK 先压缩，上游收到两次调用，之前只记了一次。

## [v1.10.2] - 2026-09-25
> 本版改动来自 Task 10.4（CMP-010 接入）。

### 修改
- **REQ-005 提交**：按 REQ-010 选模型档案，撤掉 Phase 8 的「订阅下选模型 id」过渡清单（`/api/agent-models` 下线，提交只收档案 id）。
- **REQ-009 行为**：Agent 任务的花费明细列出档案名与模型 id；兼容端点没填单价的写「未知」。
- **REQ-010 规则 · 花费（10.4 审查后补）**：按单价折算运行中读流式事件的用量（SDK 的 assistant 消息里输出 token 只记了 1）；「未知」在抽屉顶栏、横条、④ 队列行、花费明细一致；重跑默认原档案（没记档案的老任务 = 内置订阅），没换就交给服务端按原档案。

## [v1.10.1] - 2026-09-25
> 本版改动来自 Task 10.2（运行器按档案跑）。

### 修改
- **REQ-010 规则 · 运行**：各类档案注入哪些变量、不带订阅令牌、会话级环境；任务快照与继续 / 打回 / 续跑沿用原档案（原档案没了只能重跑）；重跑可重选；①参考 选的档案记在模板上及其兜底；无原生搜索时禁用 WebSearch；三种花费口径。
- **REQ-010 看图拦截**：落在 ①参考 提交与模板重跑，不建 AgentJob。
- **REQ-010 凭据不外泄**：拦读凭据变量 / 导出环境的命令（含常见的包一层写法），保护 Claude Code 登录凭据文件；消息、停因、拦截记录里的 key 打码；说明不用 SUBPROCESS_ENV_SCRUB 的原因。
- **REQ-010 输入表**：auth token 至少 8 个可见 ASCII 字符。花费口径按每一段开跑时的单价定。
- **REQ-010 运行（10.2 审查后补）**：①参考 没选档案时记下当时的默认档案并同样验看图；官方 key 档案的 haiku 映射、兼容端点的子 Agent 模型映射写明。

## [v1.10.0] - 2026-09-25
> 本版改动来自 Phase 10 拆分与 Task 10.1（模型档案存储与接口）。

### 修改
- **REQ-010 预设**：豆包方舟端点定为 `https://ark.cn-beijing.volces.com/api/coding`（Coding Plan），模型预填 `ark-code-latest`；DeepSeek 模型预填 `deepseek-flash`，联网搜索默认开（官方文档写明）。
- **REQ-010 规则**：档案管理（内置不可改删、删默认回订阅、在用不能删、改连接字段清已验证、token 打码与沿用）；测试连接的做法。
- **REQ-010 规则（10.1 审查后补）**：可添加「本机订阅 · 指定模型」档案；打开看图也清「已验证」；token 只收可见 ASCII；测试连接要求回的是 Messages API 消息，测试途中改过的档案不落结论。
