...
> 版本 v1.9.7 · 2026-09-24 · 内核：Hypit 0.2.6（本地副本 `hypit-main/`）· 技术调研见 `Hypit-Research.md`
...
- 素材审核（SCREEN-007）：
  ...
  - 重跑：Agent 停下（失败 / 熔断 / 中断）、素材待审、或出片失败的变体可以重跑；清掉 Agent 的稿子、清单与抓来的图（用户替换过的留下），运行文件清掉、素材要重新审，按原 brief 与模型开新会话。
