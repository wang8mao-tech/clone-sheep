...
// 放宽到 3 秒（11.4 第八轮审查 L12）。代价：页面自己在轮询时（估价卡、复刻、⑤ 成片、④ 队列 3 秒一轮，出片卡 2 秒），
// 默认等待也等得到下一轮——要证明「是事件触发的、不是轮询等到的」，要么让夹具处在不轮询的状态（④ 队列那条用待确认花费），
// 要么那条等待自己写比当时开着的轮询间隔还短的 timeout（CloneStep 估价事件那条；11.4 第九、十轮审查 R9-M1、R10-M1）
