...
- 验收时浏览器请求偶发卡住几十秒（Phase 9、10 各一次，只在无头 Chrome + vite 代理连跳页面后见过），疑似旧页面 SSE 占满同源 6 连接，没定性 → 下次真机抓 CDP 网络事件；若坐实，生产期由后端静态托管、或合并 SSE 为单连接。
