...
/** CMP-004 并排同步播放器：拖动 / 倍速 / 缓冲联动 / 声音来源 / 逐帧 / 布局（REQ-004、AC-011）。边界情况在 SyncPlayers.edge.test.tsx */
...
describe("逐帧", () => {
  ...
  it("给了 fps（25）就按 1/25 秒一步，进度条方向键也是一帧", () => {
    const { left } = mount(undefined, [12, 12], 25);
    fireEvent.keyDown(screen.getByRole("group", { name: "并排播放器" }), { key: "." });
    expect(left.currentTime).toBeCloseTo(1 / 25, 6);
