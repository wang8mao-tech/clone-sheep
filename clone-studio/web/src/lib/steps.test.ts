...
function states(over: Partial<StepInput> = {}): Record<StepKey, StepState> {
  const steps = deriveSteps({ status: "importing", hasSource: false, ...over });
...
describe("deriveSteps", () => {
  ...
  it("没通过验货前，④变体 与 ⑤成片 都锁着", () => {
    for (const status of ["importing", "cloning", "awaiting_review", "failed"] as const) {
      const s = states({ status, hasSource: true });
  ...
  it("复刻片已出但还没验货时，⑤成片 仍然锁着", () => {
    expect(states({ status: "awaiting_review", hasSource: true }).outputs).toBe("locked");
  });

  it("序号与名称按 Design-Brief 的五步顺序", () => {
    const steps = deriveSteps({ status: "approved", hasSource: true });
    expect(steps.map((s) => `${s.index}${s.label}`)).toEqual(["1参考", "2复刻", "3验货", "4变体", "5成片"]);
  });

  it("只有未解锁的步骤不可点", () => {
    const steps = deriveSteps({ status: "awaiting_review", hasSource: true });
...
describe("defaultStep", () => {
  const at = (over: Partial<StepInput>) => defaultStep(deriveSteps({ status: "importing", hasSource: false, ...over }));
  ...
  it("都完成了就停在能进的最后一步", () => {
    ...
    expect(at({ status: "approved", hasSource: true })).toBe("outputs");
  });
  ...
  it("永远不会返回一个进不去的步骤", () => {
    const cases: StepInput[] = [
      { status: "importing", hasSource: false },
      { status: "cloning", hasSource: true },
      { status: "awaiting_review", hasSource: true },
      { status: "approved", hasSource: true },
      { status: "failed", hasSource: false },
      { status: "failed", hasSource: true },
    ];
  ...
  it("一步都进不去时返回 undefined，而不是硬塞一个进不去的", () => {
    const allLocked = deriveSteps({ status: "importing", hasSource: false }).map((s) => ({
      ...s,
